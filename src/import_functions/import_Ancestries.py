...
def buildAncestryJson(array, charJson):
    ...
    while resume:
        ...
        if array[index].startswith("trait") and len(charJson[name]["addLanguages"]) == 0:
            charJson[name]["trait"].append(array[index].split(":")[1])
...
def importHeritage(array, charJson):
    ...
    while index < len(array) and array[index] != "eof":
        if array[index].startswith("Source") and item["source"] == "":
            item["source"] = array[index].strip("Source ")
            index += 1
            write = True
        if write and not array[index].startswith("Heading:"):
            item["description"] += array[index]
        if array[index].startswith("Heading:Feats"):
            write = False
            item["feats"] = array[index+1:len(array) - 1]
        if array[index].startswith("Heading:") and not array[index].startswith("Heading:Feats"):
            write = False
            #print(f"\tBuild Ability: {array[index]}")
            array[index] = array[index].replace("Heading:", "Ability:")
            item["abilities"], index = buildAbility(array[index:], index)
        index += 1
    ...
    # Save and add heritage to ancestry
    ...
    if len(extras) != 0:
        ...
        while index < len(extras):
            ...
            #print(f"{nextline} : {nextline in newDict}")
                if nextline in newDict:
                    extras[index] = extras[index].replace("Heading:", "Ability:")
                    ability, index = buildAbility(extras[index:], index)
                    newDict[section]["ability"] = ability
                elif "Table" in nextline:
                    extras[index] = extras[index].replace("Heading:", "Ability:")
                    ability, index = buildAbility(extras[index:], index)
                    newDict[section]["ability"] = ability
                else:
...
def buildAbility(abilities, prev):
    ...
    if ability.startswith("Table"):
        ...
        abilityDict[ability]["source"] = ""
        index = 0
        while index < len(abilities):
            #print(f"{abilities[index]}")
            if abilities[index].startswith("Source") and abilityDict[ability]["source"] == "":
                abilityDict[ability]["source"] = abilities[index].strip("Source ")
            elif abilities[index].startswith("Body:"):
                abilityDict[ability]["description"] += f"{abilities[index].split(':')[1]}\n"
            elif abilities[index].startswith("Heading"):
                index += 1
                break
            elif abilities[index].startswith("Trigger"):
                abilityDict[ability]["trigger"] = abilities[index]
            elif abilities[index].startswith("trait"):
                abilityDict[ability]["trait"].append(abilities[index].split(":")[1])
            else:
